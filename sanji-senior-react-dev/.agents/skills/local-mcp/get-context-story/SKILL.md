...
## Description
Kỹ năng này chịu trách nhiệm truy xuất toàn bộ dữ liệu nghiệp vụ và ngữ cảnh của một User Story từ kho lưu trữ thông tin dự án, làm cơ sở để phân rã nhiệm vụ và thiết kế giải pháp kỹ thuật Frontend.
...
4. **Bước 4:** Nếu MCP thiếu Detailed API, fallback Project Workspace. Trong Task Proposal phase chỉ được dùng API source mà Human chỉ rõ trong `docs/epic/<EPIC>/approval.md`.
5. **Bước 5:** Approved planning source không thay thế Detailed API khi code; execution tích hợp endpoint mới vẫn phải có Detailed API và Task Approval.
...
| Thiếu Detailed API và không có approved source | Contract chưa đủ để phân Task | Dừng và yêu cầu Detailed API hoặc nguồn contract được Human duyệt; không tự suy đoán. |
